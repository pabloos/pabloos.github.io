...
`int`. Not a type parameter — Go did not have them until 1.18, two years later. A pipeline library needs to say *a stage takes an `I` and produces an `O`*, and in 2020 there was no way to say that. The options were `interface{}` with a type assertion at every stage boundary, code generation, or picking one concrete type and pretending it stood for all of them. I picked `int` and moved on, because the alternatives make the thing you are trying to standardise worse than the hand-rolled version it was meant to replace.

So the library stalled, which was fair enough. Then generics landed in March 2022 and I did what a lot of people did: waited to see whether the community would actually adopt them or treat them as a footnote. By the time that question had clearly resolved, the post had been sitting in a drawer for long enough that reopening it felt like archaeology.

### What the sketch actually built

Strip the standardisation away and what is underneath is a chain of functions, each taking a channel and returning a channel. Every one of them looked like this — read a value, transform it, send it on:
...
Count the goroutines in that picture and you get five. Now count the values moving through it at any instant, and you also get five — one sitting on each channel. The channels are unbuffered, so a stage cannot take a second value until the next stage has taken the first off its hands. The whole chain moves in lock-step, at the speed of whichever stage is slowest.

That is the part worth saying out loud, because it took me years to see it: **the pipeline looked concurrent and had the throughput of a `for` loop.** Five goroutines, a diagram with arrows, channels everywhere — and every value walking the whole chain before the next one starts.
...
I had taken the skeleton and left behind the machinery that makes the skeleton survive contact with anything. Not out of laziness: in this design every one of those answers is wired into the stages by hand, and re-wired whenever the pipeline changes. Fan-out is not a number you raise, it is a `merge` plus a decision about ordering, written again for each pipeline. Cancellation is a `done` channel threaded into every stage and selected on at every send — miss one and you have a leak that surfaces under load, months later.

Four items, four rewrites, every one of them touching every stage. Not impossible, then. Just expensive enough that you write "TODO" and publish anyway.

The cause underneath all four is the same: the stages own the concurrency, so anything that changes how the concurrency behaves has to reach inside all of them. And that is the second problem, the one I was never blaming. The two are worth separating cleanly, because I spent years conflating them:

- **Generics blocked the library.** Real blocker, out of my hands, fixed by Go 1.18.
- **The stages owning the concurrency blocked the TODO list.** My decision, taken on the first page, and type parameters would not have touched it. A generic `stage[I, O] func(<-chan I) <-chan O` is the same trap with better types.
...
What I should have been asking in 2020, and was not: **who owns the concurrency?** In the sketch the answer is that each stage owns its own, permanently, and the caller fixes the topology by nesting. It is decided the moment you write the stage, and nobody can change it afterwards — including you, six years later, with the whole afternoon free.

That was the question worth waiting on, and I was waiting on the wrong one. On Thursday I will take apart the library I finally wrote, [flow](https://github.com/pabloos/flow), which starts from the opposite premise: you implement the ends, and the library owns the middle. Generics are what let me write its types down. The premise is what made the TODO list evaporate.
